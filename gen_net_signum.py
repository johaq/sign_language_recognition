...
class NetGen:
    ...
    def get_std_net(self, latent_dim=1024) -> K.Sequential:
        ...
        decoder_input = K.layers.Input(shape=(None, 765))
        decoder_lstm = LSTM(latent_dim, return_sequences=True, return_state=True)
        decoder_output, _, _ = decoder_lstm(decoder_input, initial_state=encoder_states)
        decoder_dense = K.layers.Dense(765, activation='softmax')
        ...
        name = self.generate_netname(latent_dim, "stdLSTMsig")
        return model, name
    ...
    def get_std_conv_net(self, latent_dim=1024) -> K.Sequential:
        ...
        image_conv = K.layers.TimeDistributed(K.layers.Conv2D(filters=3, kernel_size=(10, 10), strides=4))
        ...
        name = self.generate_netname(latent_dim, "convLSTMsig")
        return model, name
    ...
    def get_deep_conv_net(self, latent_dim=1024) -> K.Sequential:
        ...
        name = self.generate_netname(latent_dim, "deepConvLSTMsig")
        return model, name
    ...
    def get_std_conv_merge_net(self, latent_dim=1024) -> K.Sequential:
        ...
        name = self.generate_netname(latent_dim, "convMergeLSTMsig")
        return model, name
    ...
    def get_deep_conv_merge_net(self, latent_dim=1024) -> K.Sequential:
        ...
        name = self.generate_netname(latent_dim, "deepConvMergeLSTMsig")
        return model, name
    ...
    def get_vgg_19_image_net(self, latent_dim, trainable=False):
        ...
        dense = K.layers.TimeDistributed(K.layers.Dense(1000))
        encoder_input = dense(flat_out)

        encoder = LSTM(latent_dim, return_state=True)
        encoder_output, state_h, state_c = encoder(flat_out)
        encoder_states = [state_h, state_c]
        ...
        name = self.generate_netname(latent_dim, "vgg_19_retrain_sig_" + str(trainable))
        return model, name
