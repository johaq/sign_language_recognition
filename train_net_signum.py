...
class NetTrain:
    def __init__(self, model_path, data_location, data_name, latent_dim=1024, arch="std"):
        self.net_generator = gen_net_signum.NetGen()
        if arch == "std":
        ...
        self.data_generator = gen_data_signum.DataGenSIGNUM(
        ...
        self.data_generator.load_dict_from_file(data_location, data_name)
    ...
    def get_index_list(self):
        indexing_parts = [i for i in range(25)]
        shuffle(indexing_parts)
        indexing = []
        for p in indexing_parts:
            indexing_recs = [i for i in range(self.data_generator.get_rec_length(p))]
            shuffle(indexing_recs)
            for r in indexing_recs:
                indexing.append([p, r])
        return indexing
    ...
    def train_model_images(self,
        ...
        for epoch in range(initial, end):
            ...
            try:
                if with_op:
                    encoder_input_data, decoder_input_data, decoder_target_data = self.data_generator.get_image_op_sample(indexing[index][0], indexing[index][1])
                else:
                    encoder_input_data, decoder_input_data, decoder_target_data = self.data_generator.get_image_sample(indexing[index][0], indexing[index][1])
            except:
                index += 1
                print("skipping")
                continue
    ...
    def train_model_mix(self,
        ...
        for epoch in range(initial, end):
            ...
            try:
                if with_op:
                    encoder_input_data_im, decoder_input_data_im, decoder_target_data_im, encoder_input_data_op= self.data_generator.get_mix_op_sample(indexing[index][0], indexing[index][1])
                else:
                    encoder_input_data_im, decoder_input_data_im, decoder_target_data_im, encoder_input_data_op= self.data_generator.get_mix_sample(indexing[index][0], indexing[index][1])
            except:
                index += 1
